...
: taskModel(model), n(), connected(false), optimize(false), duplicateOffsets {}, QRunnable() {
...
void RunnableCorelTask::run() {
  ...
  // auto current_winstate = ax.querySubObject("ActiveWindow")->property("WindowState").toInt();
  ...
  for(int i=0; i<itemCount; ++i) {
    ...
    savedName = QDir::toNativeSeparators(modelIndex.data(KKRMainWindow::AbsoluteFilePathRole).toString());
    ...
    auto alpha_image = picShape->querySubObject("Bitmap")->querySubObject("ImageAlpha");
    ...
    if(!alpha_image->isNull()) {
      ...
      QVariantList createBitmapParams = { 
          picShape->property("LeftX"), 
      ...
      auto alpha_shape = layer-> querySubObject("CreateBitmap(double, double, double, double, IDispatch*, IDispatch*)", createBitmapParams);
      n.emitMessage("Mengukir alpha channel");
      auto traceSettings = alpha_shape->querySubObject("Bitmap")->querySubObject("Trace(2)");
      ...
      if(traced->property("Type").toInt() == 7) {
        QVariantList fsParam {QVariant(), 0, true, "@fill.color.iswhite() = False"};
        auto traced_ungrouped = traced->querySubObject("UngroupEx()");
        traced_ungrouped->querySubObject("Shapes")
          ->querySubObject("FindShapes(const QString&, int, bool, const QString&)", fsParam)->dynamicCall("Delete()");
        traced = doc->querySubObject("ActiveShape");
      }
      qreal maxApprox = qMax(traced->property("SizeWidth").toDouble(), traced->property("SizeHeight").toDouble());
      auto tracedContour = traced->querySubObject("Curve")->
        querySubObject("Contour(qreal, int, int, int)", maxApprox / 15.0, 1, 1, 2);
      tracedContour->dynamicCall("AutoReduceNodes(15)");
      auto contourShape = layer->
        querySubObject("CreateCurve(&QAxObject)", tracedContour->asVariant());
      contourShape->querySubObject("Fill")->
        querySubObject("UniformColor")->dynamicCall("BWAssign(1)");
      contourShape->querySubObject("Outline")->
        dynamicCall("SetNoOutline()");
      picShape->querySubObject("Bitmap")->dynamicCall("ConvertTo(5)"); // As CMYK Image
      picShape->dynamicCall("AddToPowerClip(&QAxObject)", contourShape->asVariant());
...
void RunnableCorelTask::optimization(bool n) {
  optimize = n;
}
