...
namespace KKRMainWindowInternal {
  ...
  class Delegate : public QStyledItemDelegate {
    public:
      Delegate(KKRMainWindow *parent): kkr(parent), val(new Validator(parent, this)), QStyledItemDelegate(parent) {};
      ~Delegate() {};
...
: ui(new Ui::KKRMainWindow), 
  ...
  connect(ui->listView->selectionModel(), &QItemSelectionModel::selectionChanged, this, &KKRMainWindow::onListViewSelectionChanged);
...
void KKRMainWindow::on_actionImportFile_triggered() {
  // import files
  auto files = QFileDialog::getOpenFileNames(this, "Pilih file untuk diproses", "", "File Trans (*.png *.webp)");
  if(!files.isEmpty()) {
    addFiles(files);
  }
}
...
void KKRMainWindow::on_actionImportDirektori_triggered() {
  ...
  QString dir = QFileDialog::getExistingDirectory(this, "Pilih folder untuk diproses");
  if(!dir.isEmpty()) {
    QStringList files;
...
void KKRMainWindow::on_actionScan_triggered() {
  ...
  QString rootDir = QFileDialog::getExistingDirectory(this, "Pilih folder untuk diproses");
  if(!rootDir.isEmpty()) {
    QFileInfo rootInfo(rootDir);
    QDirIterator dirIterator(rootDir, {"*.png", "*.webp"}, QDir::Files, QDirIterator::Subdirectories);
    QPixmap pix;
    bool pixOk = false;
    while(dirIterator.hasNext()) {
      ...
      if(!pix.isNull()) {
        ...
        item->setData(pix.scaled(128, 128, Qt::KeepAspectRatio, Qt::SmoothTransformation), Qt::DecorationRole);
        ...
        item->setData(QString("AN: %1\nText: %2").arg(info.dir().dirName(), info.baseName()), Qt::ToolTipRole);
...
void KKRMainWindow::addFiles(const QStringList& listFiles) {
  for(auto filePath : listFiles) {
    ...
    if(!pix.isNull()) {
      ...
      item->setData(QString("AN: %1\nText: %2").arg(finfo.dir().dirName(),finfo.baseName()), Qt::ToolTipRole);
      item->setData(QColor::fromCmyk(0, 0, 0, 0, 255), ColorDataRole);
      item->setData(ColorManagement::proof_of(QColor::fromCmyk(0, 0, 0, 0, 255)), Qt::BackgroundRole);
      smodel->appendRow(item);
    }
  }
}
    ...
    if(ev->type() == QEvent::KeyPress) {
        ...
        if(cIndex.isValid()) {
          ...
          if(ke->modifiers() & Qt::ShiftModifier) {
            if(row - 1 >= 0) {
              ...
              ui->listView->setCurrentIndex(cIndex.siblingAtRow(smodel->rowCount() - 1));
            }
          } else {
      ...
      for(auto url : drop->mimeData()->urls()) {
        if(url.isLocalFile()) {
          ...
          if(finfo.isDir()) {
            QDirIterator dirIterator(localFile, {"*.png", "*.webp"}, QDir::Files, QDirIterator::Subdirectories);
            while(dirIterator.hasNext()) {
              dirIterator.next();
            ...
            if(fileName.endsWith(".png", Qt::CaseInsensitive) || fileName.endsWith(".webp", Qt::CaseInsensitive)) {
...
void KKRMainWindow::changeBackgroudItemRequest(QStandardItem* item) {
  ...
  // cd->setOptions(QColorDialog::ShowAlphaChannel | QColorDialog::NoButtons | QColorDialog::DontUseNativeDialog);
  CMYKColorDialog *cd = new CMYKColorDialog(cl, this);
  cd->setAttribute(Qt::WA_DeleteOnClose);
  connect(cd, &CMYKColorDialog::currentColorChanged, this, &KKRMainWindow::onColorSelected);
  cd->adjustSize();
  cd->open();
}
...
void KKRMainWindow::onListViewSelectionChanged(const QItemSelection& sel, const QItemSelection& des) {
...
void KKRMainWindow::on_listView_customContextMenuRequested(const QPoint& pt){
  ...
  if(indexAt.isValid()) {
    ...
    connect(hapus, &QAction::triggered, [this, &indexAt]()
      { smodel->removeRow(indexAt.row()); });
  }
  auto reset = contextMenu.addAction("Reset");
  reset->setToolTip("Hapus semua tugas");
  connect(reset, &QAction::triggered, [this](){
    if(QMessageBox::question(this, "Konfirmasi", "Hapus semua item?") == QMessageBox::Yes) {
      smodel->clear();
    }
  });
...
void KKRMainWindow::on_mulaiButton_clicked() {
  QStandardItem *item;
  QModelIndexList emptyCustomer,
                  emptyInformation,
                  emptyDrawText;
  if(smodel->rowCount() == 0) {
    QMessageBox::information(this, "Kesalahan", "Belum ada data untuk di proses");
    return ;
  }
  ...
  if(!emptyDrawText.isEmpty()) {
    errMessage += QString("%1 item tidak memiliki parameter Text").arg(emptyDrawText.size());
  }
  if(!errMessage.isEmpty()) {
    QMessageBox::information(this, "Data tidak lengkap", QString("Error :\n%1").arg(errMessage));
    return ;
  }
...
QWidget* KKRMainWindowInternal::Delegate::createEditor(QWidget* parent, const QStyleOptionViewItem &opt, const QModelIndex& ix) const {
...
void KKRMainWindow::on_aboutQt_triggered() {
  qApp->aboutQt();
}
